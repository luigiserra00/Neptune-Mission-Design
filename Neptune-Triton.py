...
simulation_start_epoch = DateTime(2073, 1, 1).epoch()
simulation_end_epoch   = DateTime(2074, 1, 1).epoch()

# Define string names for bodies to be created from default.
bodies_to_create = ["Sun","Neptune","Triton"]

# Use "Earth"/"J2000" as global frame origin and orientation.
global_frame_origin = "Triton"
...
mu_Triton = bodies.get_body("Triton").gravitational_parameter

initial_state = element_conversion.keplerian_to_cartesian([a, e, inc, raan, arg_per, true_anomaly], mu_Triton)
...
dependent_variables_to_save = [
    ...
    propagation_setup.dependent_variable.keplerian_state("Delfi-C3", "Triton"),
    propagation_setup.dependent_variable.latitude("Delfi-C3", "Triton"),
    propagation_setup.dependent_variable.longitude("Delfi-C3", "Triton"),
...
plt.plot(time_hours, acceleration_norm_pm_mars, label='PM Triton')
...
plt.plot([time_hours[0],time_hours[-1]], [r_Triton, r_Triton], label='Triton Radius', linestyle='--', color='black')
plt.xlabel('Time [hr]')
