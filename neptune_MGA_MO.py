...
class TransferTrajectoryProblem:
    ...
    def get_nobj(self):
        # Explicitly state the number of objectives
        return 2
    ...
    def fitness(self, trajectory_parameters: List[float]) -> list:
        ...
        try:
            ...
            tof = transfer_trajectory.time_of_flight
...
# Fix seed
optimization_seed = 75000

# Create pygmo algorithm object
algo = pg.algorithm(pg.moead_gen(gen=1, seed=optimization_seed, weight_generation="grid",decomposition="tchebycheff",neighbours=20,CR=1,F=0.5,eta_m=40,realb=0.6,limit=8,preserve_diversity=True))
algo = pg.algorithm(pg.nsga2(gen=number_of_generations,seed=optimization_seed)
...
for i in range(number_of_evolutions):
    ...
    print("Evolving population, generation %i/%i", (i, number_of_evolutions-1), end="\r")
    pop = algo.evolve(pop)
...
pickle.dump([fitness_list, individuals_list], open("results.pkl", "wb"))
...
print('Jupiter-Neptune time of flight [days]: ', best_decision_variables[5])
